...
use janu::net::utils::resource_name;
use janu::net::{encoding, ZBuf, ZInt};
use janu::{Timestamp, TimestampId, Value, ZError, ZErrorKind, ZResult};
...
// a structure holding a janu path (absolute) and the corresponding file-system path (including the base_dir)
...
impl FilesMgr {
    ...
    // Encoding and timestamp are retrieved from the data_info_mgr if file was put via janu.
    ...
    async fn get_encoding_and_timestamp(
        ...
        match self.data_info_mgr.get_encoding_and_timestamp(file).await? {
            ...
            None => {
                ...
                // transform mime type into janu encoding flag if possible
                ...
                Some(Ok(e)) => {
                    if e.file_type().is_dir() {
                        ...
                        if let Some(s) = fspath.to_str() {
                            // zpath is the file's absolute path stripped from base_dir and converted as janu path
                            // note: force owning to not have fspath borrowed
                            let zpath =
                                Cow::from(fspath_to_zpath(&s[self.base_dir_len..]).into_owned());
                            // convert it to janu path for matching test with zpath_expr
...
pub(crate) fn fspath_to_zpath(fspath: &str) -> Cow<'_, str> {
    const JANU_SEP: &str = "/";
    Cow::from(fspath.replace(std::path::MAIN_SEPARATOR, JANU_SEP))
}
