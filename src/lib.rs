...
use janu::net::{DataInfo, Sample};
use janu::{Change, ChangeKind, Properties, Selector, Value, ZError, ZErrorKind, ZResult};
use janu_backend_traits::*;
use janu_util::{janu_home, zerror, zerror2};
...
/// The default root (whithin janu's home directory) if the ZBACKEND_FS_ROOT environment variable is not specified.
...
pub fn create_backend(_unused: &Properties) -> ZResult<Box<dyn Backend>> {
    ...
    let root = if let Some(dir) = std::env::var_os(SCOPE_ENV_VAR) {
        ...
        let mut dir = PathBuf::from(janu_home());
    ...
    let admin_status = janu::utils::properties_to_json_value(&properties);
...
impl Backend for FileSystemBackend {
    ...
    async fn create_storage(&mut self, props: Properties) -> ZResult<Box<dyn Storage>> {
        ...
        let admin_status = janu::utils::properties_to_json_value(&props);
    ...
    async fn reply_with_file(&self, query: &Query, zfile: &ZFile<'_>) {
        match self.files_mgr.read_file(zfile).await {
            Ok(Some((value, timestamp))) => {
                ...
                // append path_prefix to the janu path of this ZFile
